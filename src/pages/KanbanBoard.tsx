...
const DraggableDemandCard = ({ demand, onEdit }: DraggableDemandCardProps) => {
  ...
  return (
    <Card ref={setNodeRef} style={style} {...attributes} {...listeners} onDoubleClick={() => onEdit(demand)} className="cursor-grab hover:shadow-lg transition-all duration-300 group mb-3 overflow-hidden">
  ...
  return (
    <div ref={setNodeRef} className={cn("flex flex-col bg-muted/30 p-3 rounded-lg shadow-sm min-h-[200px] w-full", isDraggingOver && "border-2 border-dashed border-primary/50 bg-primary/5")}>
      ...
      <div className="space-y-2 flex-1">
        <SortableContext items={demands.map((d: any) => d.id)} id={id}>{demands.map((demand: any) => <DraggableDemandCard key={demand.id} demand={demand} onEdit={onEdit} />)}</SortableContext>
      </div>
