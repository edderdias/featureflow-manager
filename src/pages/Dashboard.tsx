    ...
    <div ref= { setNodeRef } style = { style } {...attributes } {...listeners }>
      <Card onDoubleClick={ () => onEdit(demand) } className = "cursor-grab hover:shadow-lg transition-all duration-300 group mb-3 overflow-hidden" >
      {
        demand.tags && demand.tags.length > 0 && (
          <div className="px-3 pt-2 pb-0"> <div className="flex flex-wrap gap-1"> {
            demand.tags.slice(0, 2).map((tag) => <Badge key={ tag } variant = "outline" className = "text-[9px] h-4 px-1 truncate max-w-[80px]" > { tag } < /Badge>)}</div > </div>
            )
          }
...
const KanbanBoard = () => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    console.log('handleDragEnd ----> ', event, ' - ', active, ' - ', over)
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" >
                        {
                          columns.map((status) => {
                            ...
                            return <KanbanColumn key={ status } id = { status } title = { statusLabels[status]} demands = { display } totalDemandsCount = { colDemands.length } onEdit = { handleEdit } showLoadMore = { status === "done" && colDemands.length > visibleDoneDemandsCount
